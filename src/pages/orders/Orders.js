...
export default function Orders() {
  ...
  useEffect(() => {
    axios.get('http://localhost:3002/orders',{headers:{token:token}}).then(res=>setProducts(res.data))
  },[])
  ...
  const handlechange = (value) => {
    if (value === 0) {
      axios.get(BASE_URL + ORDERS,{headers:{token:token}}).then(res => setProducts(res.data))
      return
    }
    axios.get(BASE_URL+ORDERS+`?orderStatus=${value}`).then(res=>setProducts(res.data))
